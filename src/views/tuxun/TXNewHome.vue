<template>
  <div class="container" :style="{'background-image':'linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.6)),url('+imgOrigin+ this.backgroundImage +')','background-size':'cover','background-position': 'center'}">
    <div class="game">
      ...
      <div @click="toFirstTournament" class="info">玩家图寻第四届组队赛, 5月13日开赛，欢迎报名～</div>
...
export default {
  ...
  methods:{
    ...
    toFirstTournament() {
      try {
        window.flutter_inappwebview.callHandler('toAppPost', {postId: '1259223'});
      } catch (e) {
        tuxunOpen('https://choa.fun/p/1259223');
      }
    },
    raiseVIP() {
      this.doLoginStatus().then(res=>{
