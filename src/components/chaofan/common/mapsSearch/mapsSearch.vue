<template>
  <div v-if="show" class="cover">
    <section class="section" style="display: block;overflow:scroll; overscroll-behavior: contain; height: 100%;    scrollbar-width: none;">
      <div style="font-size: 20px; color: white; padding-top: 2rem">题库搜索<el-button type="primary" style="margin-left: 10px" @click="close" round>关闭</el-button></div>
        ...
        <div @click.stop="toMapsDetail(item)" style="display: flex;  justify-content: space-between;">
          <div style="height: 100%;display: flex; color: white">
            <div>
              {{item.name}}
              <div>
                <span style="font-size: 12px;bottom: 0px; color: grey">(<span v-if="item.pcount">地点: {{item.pcount}} | </span>人次: {{item.players}} <span v-if="item.canMove"> | 可移动 </span>)</span>
              </div>
            </div>
          </div>
          <div v-if="callBack">
